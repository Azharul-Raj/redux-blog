...
import updateAContent from '../../redux/thunk/contentThunk/updateAContent';
...
const AddContent = () => {
    ...
    const onSubmit = data => {
        ...
        if (location.pathname = "/update_content/:id") {
            return dispatch(updateAContent(content,id))
        }
        else {
            return dispatch(AddAContent(content))
        }
    } 
...
<form onSubmit={handleSubmit(onSubmit)} className='mx-5 lg:mx-20'>
  ...
  <button type="submit" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">{location.pathname==="/add_content"?"Submit":"Update Content"}</button>
</form>
